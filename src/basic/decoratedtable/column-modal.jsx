 ...
 * @Last Modified time: 2019-06-04 17:06:36
 */
...
class ColumnModal extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        let {visible, columns = null, all=[] } = nextProps;
        if (visible !== this.state.visible) {
            let selectedColumns = columns ? columns.map(c => c.dataIndex) : [];
            this.setState({
                visible,
                selectedColumns,
                checkAll: all && selectedColumns && selectedColumns.length === all.length
            });
        }
    }
    handleCheckAll(e) {
